# --- BASE DE DATOS PROTEGIDA DE TIGAFAB ---
# Este archivo contiene SOLO el texto y los servicios.
# No tocar para cambios de diseño.

TRANSLATIONS = {
    'es': {
        'nav_home': "Inicio", 'nav_services': "Servicios", 'nav_contact': "Contacto", 'nav_lang': "Idioma",
        'hero_title': "Excelencia en Traducción", 'hero_subtitle': "ÁRABE • ESPAÑOL • INGLÉS • ALEMÁN • FRANCÉS",
        'home_cta_btn': "SOLICITAR PRESUPUESTO",
        
        # TEXTOS HOME (MASIVOS COMPLETOS)
        'h1': "Somos un despacho de traductores e interpretes oficiales, jurados, nativos profesionales, encabezados por doña Fatima Benamar, con más de 15 años de experiencia. Ofrecemos a nuestros clientes un servicio rápido, personalizado, puntualidad, máxima calidad, confidencialidad, asesoramiento, trato directo y tarifas ajustadas.",
        'h2': "OFRECEMOS un servicio integral en traducción jurada y asesoramiento para la tramitación de todos sus documentos.",
        'h3': "A través de nuestro asesoramiento y atención personalizada le garantizamos una tramitación correcta. De esta forma se ahorrará mucho tiempo, dinero y disgustos.",
        'h4': "Siempre satisfacemos a nuestros clientes. Y nos comprometemos a entregar el trabajo en el plazo acordado.",
        'h5': "Cada vez son más las empresas españolas que se proyectan al exterior que cuentan con nuestros servicios de traducción, gestión y asesoramiento para su expansión en el mercado internacional, ofrecemos servicio de introducción de empresas constructoras en el país LIBIA, por ello contamos con agentes y empresarios en Libia, con proyectos adjudicados, proporcionamos todo el asesoramiento, consejos y traductores en Libia, para asistir a sus reuniones en sus viajes en este país.",
        'h6': "Nuestras tarifas de precios sin competencia y nuestro equipo técnico especializado hacen de nosotros un punto de referencia en el sector. Hemos realizado proyectos para diferentes empresas. Estamos avalados por muchas grandes empresas españolas e internacionales.",
        
        # SERVICIOS (LISTA MAESTRA COMPLETA - 40+ SERVICIOS)
        'srv_header_main': "Nuestras Especialidades",
        'srv_header_sub': "Servicios de élite en Traducción, Gestión y Asesoramiento Internacional",
        'srv_jurada_title': "Traducciones Juradas",
        'srv_jurada_list': [
            "Pasaportes al Árabe", "Escrituras y Contratos", "Poderes Notariales", "Estatutos de Sociedades", 
            "Auditorías y Cuentas Anuales", "Autorizaciones y Actas", "Certificados de Cámara y Mercantil", 
            "Certificados de Origen y Sanitarios", "Facturas Comerciales", "Certificados de Nacimiento y Divorcio", 
            "Certificados Penales y Médicos", "Títulos y Expedientes Académicos"
        ],
        'srv_tecnica_title': "Traducciones Técnicas",
        'srv_tecnica_list': [
            "Presentaciones y Catálogos", "Planes de Negocio", "Manuales de Ingeniería", "Páginas Web", 
            "Listados de Precios", "Correspondencia y Cartas", "Informes y Notas Técnicas"
        ],
        'srv_organismos_title': "Gestión ante Organismos",
        'srv_organismos_list': [
            "Ministerio de Asuntos Exteriores", "Ministerio de Justicia", "Ministerio de Sanidad y Agricultura", 
            "Cámara de Comercio de Madrid", "Colegio de Notarios de Madrid", 
            "Embajadas (Libia, Argelia, Irán, Egipto, China, etc...)"
        ],
        'srv_visados_title': "Gestión de Visados",
        'srv_visados_list': [
            "Tramitación en consulados extranjeros", "Libia, Argelia, Irán, Egipto...", 
            "Gestión de impresos oficiales", "Asesoramiento íntegro en requisitos"
        ],
        
        'srv_libia_title': "Asesoramiento Especializado LIBIA",
        'srv_libia_desc': "Introducimos y asesoramos a su empresa en el complejo mercado LIBIO. Contamos con apoyo técnico en destino, contactos reales con agentes locales y acompañamiento bilingüe en reuniones estratégicas para asegurar el éxito de sus proyectos.",
        'srv_libia_btn': "Solicitar Información Mercado Libio",
        
        # CONTACTO (DATOS COMPLETOS RECUPERADOS)
        'hero_contact_title': "Contacte con Nosotros",
        'contact_title': "Nuestras Oficinas y Delegaciones",
        'info_address_title_1': "Delegación Fuenlabrada",
        'info_address_text_1': "C/ de las Ciencias, 51. 28942 Fuenlabrada, Madrid",
        'info_address_title_2': "Delegación Madrid Capital",
        'info_address_text_2': "C/ Mauricio Legendre, 5 - Local 11. 28046 Madrid",
        'info_email_title': "Correos Electrónicos",
        'info_emails': ["fatima@tigafab-traductores.com", "fatima.benamar@yahoo.es", "info@tigafab.com"],
        'info_phone_title': "Teléfonos de Contacto",
        'info_phones': ["+34 91 606 20 20 (Oficina)", "+34 663 11 45 46 (Móvil)"],
        'info_fax_title': "Fax Corporativo",
        'info_fax': "+34 914 86 47 50",
        'form_name': "Nombre Completo",
        'form_email': "Correo Electrónico",
        'form_phone': "Teléfono de contacto",
        'form_message': "Díganos cómo podemos ayudarle...",
        'form_btn': "ENVIAR SOLICITUD",
        
        # REVIEWS
        'reviews_title': "Confianza Global",
        'exp_verificadas': "EXPERIENCIAS VERIFICADAS",
        
        # FOOTER
        'footer_rights': "© 2026 TIGAFAB S.L. Boutique de Traducción Jurada.",
        'legal_title': "Aviso Legal",
        'privacy_title': "Política de Privacidad"
    },
    'en': {
        'nav_home': "Home", 'nav_services': "Services", 'nav_contact': "Contact", 'nav_lang': "Language",
        'hero_title': "Excellence in Translation", 'hero_subtitle': "ARABIC • SPANISH • ENGLISH • GERMAN • FRENCH",
        'home_cta_btn': "GET A QUOTE",
        'h1': "Professional translation firm led by Ms. Fatima Benamar, with over 15 years of experience. We offer fast, personalized service, top quality, and fair rates.",
        'srv_header_main': "Our Specialties",
        'srv_jurada_title': "Sworn Translations",
        'srv_jurada_list': ["Passports to Arabic", "Contracts & Deeds", "Powers of Attorney", "Articles of Association", "Annual Accounts", "Birth/Divorce Certs", "Criminal Records", "Academic Degrees"],
        'srv_libia_title': "Libya Market Advisory",
        'srv_libia_desc': "We bridge European companies with the LIBYAN market through strategic support and local bilingual guidance.",
        'form_btn': "SUBMIT REQUEST",
        'footer_rights': "© 2026 TIGAFAB S.L. Sworn Translation Boutique.",
        'legal_title': "Legal Notice", 'privacy_title': "Privacy"
    },
    'ar': {
        'nav_home': "الرئيسية", 'nav_services': "خدماتنا", 'nav_contact': "اتصل بنا", 'nav_lang': "اللغة",
        'hero_title': "رواد الترجمة المعتمدة", 'hero_subtitle': "العربية • الإسبانية • الإنجليزية • الألمانية • الفرنسية",
        'home_cta_btn': "طلب عرض سعر",
        'h1': "مكتب ترجمة ومترجمين شفويين محلفين ومعتمدين رسمياً، بخبرة تمتد لأكثر من ١٥ عاماً لمساعدة الشركات والأفراد. نحن نقدم دقة وسرعة وسرية تامة.",
        'h2': "خدمات متكاملة في الترجمة المعتمدة وإدارة المعاملات لجميع مستنداتكم.",
        'srv_header_main': "تخصصاتنا المهنية",
        'srv_header_sub': "خدمات النخبة في الترجمة المعتمدة والاستشارات الدولية",
        'srv_jurada_title': "الترجمة المحلفة والمخطوطات",
        'srv_jurada_list': [
            "جوازات سفر إلى العربية", "عقود ومواثيق رسمية", "توكيلات قانونية", "النظام الأساسي للشركات", 
            "التدقيق المالي والحسابات السنوية", "تفويضات ومحاضر رسمية", "شهادات الغرف التجارية والسجل التجاري", 
            "شهادات المنشأ والشهادات الصحية", "فواتير تجارية", "شهادات الميلاد والطلاق", 
            "شهادات السوابق الجنائية والتقارير الطبية", "الشهادات والملفات الأكاديمية"
        ],
        'srv_tecnica_title': "الترجمة التقنية والمتخصصة",
        'srv_tecnica_list': [
            "عروض تقديمية وكتالوجات", "خطط عمل", "أدلة هندسية", "صفحات ويب", 
            "قوائم أسعار", "مراسلات وخطابات", "تقارير وملاحظات فنية"
        ],
        'srv_organismos_title': "إجراءات لدى الهيئات الرسمية",
        'srv_organismos_list': [
            "وزارة الشؤون الخارجية", "وزارة العدل", "وزارة الصحة والزراعة", 
            "غرفة تجارة مدريد", "نقابة الموثقين بمدريد", 
            "السفارات (ليبيا، الجزائر، إيران، مصر، الصين، إلخ...)"
        ],
        'srv_visados_title': "تنسيق وإدارة التأشيرات",
        'srv_visados_list': [
            "إجراءات المعاملات في القنصليات الأجنبية", "ليبيا، الجزائر، إيران، مصر...", 
            "إدارة النماذج الرسمية", "استشارات كاملة حول المتطلبات"
        ],
        'srv_libia_title': "استشارات متخصصة في السوق الليبي",
        'srv_libia_desc': "نحن نقدم الدعم الفني والتمثيل الاستراتيجي للشركات الراغبة في دخول السوق الليبي بكفاءة، مع خبرة واسعة في إدارة المعاملات المحلية.",
        'srv_libia_btn': "طلب معلومات حول السوق الليبي",
        # CONTACTO (ARABIC FULL RESTORATION)
        'hero_contact_title': "اتصل بنا",
        'contact_title': "مكاتبنا ووفودنا",
        'info_address_title_1': "فرع فوينلابرادا (مدريد)",
        'info_address_text_1': "C/ de las Ciencias, 51. 28942 Fuenlabrada, Madrid",
        'info_address_title_2': "فرع وسط مدريد",
        'info_address_text_2': "C/ Mauricio Legendre, 5 - Local 11. 28046 Madrid",
        'info_email_title': "البريد الإلكتروني",
        'info_emails': ["fatima@tigafab-traductores.com", "fatima.benamar@yahoo.es", "info@tigafab.com"],
        'info_phone_title': "هواتف الاتصال",
        'info_phones': ["+34 91 606 20 20 (المكتب)", "+34 663 11 45 46 (مباشر)"],
        'info_fax_title': "الفاكس",
        'info_fax': "+34 914 86 47 50",
        'form_btn': "إرسال الطلب",
        'footer_rights': "© ٢٠٢٦ تيجافاب. مكتب ترجمة دولي معتمد.",
        'legal_title': "إشعار قانوني", 'privacy_title': "سياسة الخصوصية"
    }
}

REVIEWS_DATA = [
    ("Marta (Google)", "Servicio impecable y muy profesional. Fátima resolvió la traducción jurada de mis documentos en tiempo récord para un trámite urgente. Su trato y rapidez son excelentes."),
    ("Mohamed (Google)", "Gente muy profesional y muy amable. Lo recomiendo 100% para cualquier gestión de traducción o visados. Me ayudaron con el registro en un tiempo récord."),
    ("Juan Carlos (Google)", "Atención excepcional y rapidez. Necesitaba una traducción jurada de árabe a español para el ministerio y lo tuvieron listo antes del plazo. Rigurosos."),
    ("Laila (Google)", "La mejor agencia de Madrid para árabe. Son extremadamente serios con los plazos de entrega, algo vital para licitaciones. Trato exquisito."),
    ("Raúl (Google)", "Traducciones técnicas de alta calidad. Entienden perfectamente el lenguaje de ingeniería y construcción pesada."),
    ("Constructor S.A. (Google)", "Indispensables para nuestras licitaciones en Libia. Su conocimiento del mercado nos ha permitido cerrar contratos con éxito.")
]
