...
LANG_NAMES = {'es': 'Español', 'en': 'English', 'fr': 'Français', 'ar': 'العربية'}
LANG_FOLDERS = {'es': '', 'en': 'en', 'fr': 'fr', 'ar': 'ar'}
...
def get_visa_docs_html(rel_path, lang):
    ...
    prefixes = {
        'es': "Pincha sobre el país correspondiente para poder descargar el formulario:",
        'en': "Click on the corresponding country to download the form:",
        'fr': "Cliquez sur le pays correspondant pour télécharger le formulaire:",
        'ar': "انقر على الدولة المقابلة لتحميل النموذج:",
    }
    files = [
    ...
    links = ", ".join([f'<a href="{rel_path}img/docs/{file}" target="_blank" rel="noopener noreferrer" class="visa-country-link">{country}</a>' for country, file in docs])
...
for lang in LANG_FOLDERS:
    ...
    <div data-aos="fade-right" style="position:sticky; top:120px;">
        <div style="background:rgba(255,255,255,0.02); border:1px solid var(--glass-border); padding:3.5rem; border-radius:30px; backdrop-filter:blur(10px); position:relative; overflow:hidden; transition: all 0.3s ease; cursor: pointer;" onmouseover="this.querySelector('.fatima-overlay').style.opacity='0'; this.querySelector('.fatima-overlay').style.transition='opacity 0.3s ease'; this.querySelector('.fatima-content').style.opacity='0'; this.querySelector('.fatima-content').style.transition='opacity 0.3s ease'; this.querySelector('.fatima-img').style.opacity='1'; this.querySelector('.fatima-img').style.transition='opacity 0.3s ease';" onmouseout="this.querySelector('.fatima-overlay').style.opacity='0.4'; this.querySelector('.fatima-content').style.opacity='1'; this.querySelector('.fatima-img').style.opacity='0.55';">
            <img src="{rel_path}img/fatima.jpeg" alt="Fatima Benamar Bahamad" class="fatima-img" style="position:absolute; top:0; left:0; width:100%; height:100%; object-fit:cover; opacity:0.55; z-index:0; transition: opacity 0.3s ease;">
    ...
    <section style="padding: 8rem 0; background: var(--bg-main);"><div class="container"><div class="services-creative-grid">
    ...
    thanks_url = f"https://bantawit.github.io/TigafabNewVersion/{LANG_FOLDERS[lang] + '/' if LANG_FOLDERS[lang] else ''}gracias.html"
    form_action = f"{rel_path}send-email.php" if lang != 'es' else "send-email.php"
    
    # Adjust privacy links for language folders
    privacy_text = t.get('form_privacy', '')
    if lang != 'es':
        privacy_text = privacy_text.replace('href="aviso-legal.html"', f'href="{rel_path}aviso-legal.html"')
        privacy_text = privacy_text.replace('href="politica-privacidad-cookies.html"', f'href="{rel_path}politica-privacidad-cookies.html"')
    ...
    <div class="contact-form-premium" data-aos="fade-left"><form action="{form_action}" method="POST" enctype="multipart/form-data"><div class="form-group-premium"><input type="text" name="name" placeholder="{t['form_name']}" required></div><div class="form-group-premium"><input type="email" name="email" placeholder="{t['form_email']}" required></div><div class="form-group-premium"><input type="tel" name="phone" placeholder="{t['form_phone']}"></div><div class="form-group-premium"><select name="language" style="width:100%; padding:1rem; background:rgba(255,255,255,0.05); border:1px solid var(--glass-border); border-radius:10px; color:var(--text-soft); font-size:1rem;"><option value="">{t['form_language']}</option><option value="Español">Español</option><option value="Árabe">Árabe</option><option value="Inglés">Inglés</option><option value="Francés">Francés</option></select></div><div class="form-group-premium"><textarea rows="5" name="message" placeholder="{t['form_message']}" required></textarea></div><div class="form-group-premium"><label style="display:block; margin-bottom:0.5rem; color:var(--text-soft); font-size:0.9rem;">{t['form_file']}</label><div class="file-upload-wrapper"><input type="file" name="attachments[]" id="fileInput" multiple style="display:none;"><label for="fileInput" class="file-upload-label"><i class="fas fa-cloud-upload-alt"></i><span class="file-upload-text">t-form-file-text</span></label><span class="file-name-display" id="fileName"></span></div><p style="font-size:0.8rem; color:var(--text-soft); margin-top:0.5rem;">t-form-file-info</p></div><div class="form-group-premium" style="display:flex; align-items:center; gap:0.5rem;"><input type="checkbox" name="privacy" id="privacy" required style="width:auto; margin:0;"><label for="privacy" style="color:var(--text-soft); font-size:0.85rem; margin:0;">{privacy_text}</label></div><button type="submit" class="btn-premium" style="width:100%">t-form-btn</button></form></div>
    </div></div></section>"""
    ...
    <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(450px, 1fr)); gap:40px;">
        <div class="map-box-premium" data-aos="fade-right">
            <div style="padding:2.5rem; background:rgba(255,255,255,0.02); border-bottom:1px solid var(--glass-border);">
                ...
                <a href="https://www.google.com/maps/place/Tigafab+S.L/@40.4691753,-3.8393874,22450m/data=!3m2!1e3!5s0xd422916a7dd026f:0x483687832cd1e56!4m10!1m2!2m1!1stigafab!3m6!1s0xd422916a64bc2a7:0x34396661766aaae!8m2!3d40.4691753!4d-3.6869521!15sCgd0aWdhZmFikgEKdHJhbnNsYXRvcuABAA!16s%2Fg%2F11pv28zf7r?entry=ttu&g_ep=EgoyMDI2MDUyMC4wIKXMDSoASAFQAw%3D%3D" target="_blank" class="btn-premium" style="padding: 0.5rem 1.5rem; font-size: 0.8rem; margin-top: 1.5rem; display: inline-block;">t-loc-how-to-get</a>
            </div>
            <div style="height:400px; width:100%; border-radius: 0 0 20px 20px; overflow:hidden;">
                <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3035.272308118042!2d-3.689149684292716!3d40.469240360367586!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0xd422916a7b83f21%3A0xbed48e4a72bff632!2sCalle+de+Mauricio+Legendre%2C+5%2C+28046+Madrid!5e0!3m2!1ses!2ses!4v1551108354291" width="100%" height="100%" frameborder="0" style="border:0; filter: var(--map-filter);" allowfullscreen="" loading="lazy"></iframe>
            </div>
        </div>
        <div class="map-box-premium" data-aos="fade-left">
            <div style="padding:2.5rem; background:rgba(255,255,255,0.02); border-bottom:1px solid var(--glass-border);">
                ...
                <a href="https://www.google.com/maps/place/Tigafab/@40.2790635,-3.8155003,704m/data=!3m3!1e3!4b1!5s0xd418b705bf8a0ad:0x446aecf9c1dba2db!4m6!3m5!1s0xd418b705bdaedaf:0xb627f855c450d059!8m2!3d40.2790594!4d-3.8129254!16s%2Fg%2F11gbfbp34w?entry=ttu&g_ep=EgoyMDI2MDUyMC4wIKXMDSoASAFQAw%3D%3D" target="_blank" class="btn-premium" style="padding: 0.5rem 1.5rem; font-size: 0.8rem; margin-top: 1.5rem; display: inline-block;">t-loc-how-to-get</a>
            </div>
            <div style="height:400px; width:100%; border-radius: 0 0 20px 20px; overflow:hidden;">
                <iframe src="https://maps.google.es/maps?f=q&source=s_q&hl=es&geocode=&q=C%2F+De+las+Ciencias,+51+28942+-+Fuenlabrada+(Madrid)&ie=UTF8&hq=&hnear=Calle+de+las+Ciencias,+51,+Fuenlabrada,+Madrid&t=m&ll=40.286597,-3.812428&spn=0.022916,0.055361&z=14&output=embed" width="100%" height="100%" frameborder="0" style="border:0; filter: var(--map-filter);" allowfullscreen="" loading="lazy"></iframe>
            </div>
        </div>
    </div>
    </div></section>"""
    ...
    # 7. GRACIAS (Thank you page)
    gracias = f"""<section class="hero hero-contact" style="min-height:60vh; padding:120px 0;"><div class="container" data-aos="fade-up" style="text-align:center;"><i class="fas fa-check-circle" style="font-size:5rem; color:var(--primary); margin-bottom:2rem;"></i><h1 style="font-size:3rem; margin-bottom:1.5rem;">t-thanks-title</h1><p style="font-size:1.3rem; color:var(--text-muted); max-width:600px; margin:0 auto 2rem;">t-thanks-message</p><a href="index.html" class="btn-premium" style="display:inline-block;">t-thanks-btn</a></div></section>"""
    generate_page(lang, "gracias.html", 'nav_home', gracias)
